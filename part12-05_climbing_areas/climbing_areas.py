...
def sort_by_number_of_routes(areas: list):
    ...
    def total_routes(area: ClimbingArea):
        return area.routes()

    return sorted(areas, key = total_routes)

def sort_by_most_difficult(areas: list):
    
    def route_difficulty(area: ClimbingArea):
        return area.hardest_route().grade

    return sorted(areas, key = route_difficulty, reverse = True)
